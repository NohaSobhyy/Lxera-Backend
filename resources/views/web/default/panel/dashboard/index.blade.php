            ...
            <div class="col-12  mt-35 @if ($authUser->isUser()) col-lg-5 @endif">
                ...
                <div class="bg-white dashboard-banner-container position-relative p-40 rounded-sm">
                    <h2 class="font-30 text-primary line-height-1">
                        <span class="d-block {{ auth()->user()->language === 'AR' ? 'text-left' : 'text-right' }}">{{ trans('panel.hi') }} {{ $authUser->full_name }}</span>
                    </h2>
                    @if (!$authUser->isUser())
                        <span
                    ...
                    @if ($authUser->isUser())
                        <ul class="mt-15 unread-notification-lists">
                            <h4 class="{{ auth()->user()->language === 'AR' ? 'text-left' : 'text-right' }}"> {{ trans('panel.academic_info') }}</h4>
                            <li class="mt-1 text-gray font-16 font-weight-bold {{ auth()->user()->language === 'AR' ? 'text-left' : 'text-right' }}"> {{ trans('panel.personal_card_stucode') }} :
                                {{ $authUser->user_code }}</li>
                            <li class="mt-1 text-gray font-16 font-weight-bold {{ auth()->user()->language === 'AR' ? 'text-left' : 'text-right' }}"> {{ trans('panel.personal_card_email') }}  :
                                {{ $authUser->user_code }}@anasacademy.uk</li>
                            <li class="mt-1 text-gray font-16 font-weight-bold {{ auth()->user()->language === 'AR' ? 'text-left' : 'text-right' }}">  {{ trans('panel.personal_card_password') }} :
                                SD$$2025</li>

                                @if (auth()->user()->language === 'AR')
                                    <li class="mt-1 text-gray font-16 font-weight-bold text-left">  {{ trans('panel.personal_card_current_program') }} :                                    
                                @else
                                    <li class="mt-1 text-gray font-16 font-weight-bold text-right">
                                @endif
                                ...
                                @else
                                    لم يتم التسجيل بعد
                                    @endif
                                {{ auth()->user()->language !== 'AR' ? ': ' . trans('panel.personal_card_current_program') : '' }}  
                            </li>                                
                        </ul>
                    @endif
                </div>
                    ...
                    <div
                        ...
                        <div>
                            <div class="text-center">
                                ...
                                <span
                                    class="mt-5 d-block font-30 text-secondary">{{ handlePrice($authUser->getAccountCharge()) }}</span>
                            </div>
                ...
                <div class="col-12 col-lg-3">
                    <div
                        ...
                        <div class="d-flex flex-column pt-35" style="align-items: center;">
                            ...
                            <span class="font-16 text-gray font-weight-500 text-center pb-10">
                                {{ trans('panel.microsoft_title') }} 
                            </span>
                            <a target="_blank" rel="noopener noreferrer" class="btn btn-primary mt-10" style=""
                                href="https://go.microsoft.com/fwlink/?linkid=2187217&amp;clcid=0x409&amp;culture=en-us&amp;country=us/">
                                {{ trans('panel.microsoft_download_know_how') }}
                            </a>

                            <a target="_blank" rel="noopener noreferrer" class="btn btn-primary mt-10" style=""
                                href="https://portal.office.com/">
                                {{ trans('panel.microsoft_personal_office_sign_in') }}
                            </a>
                        </div>
