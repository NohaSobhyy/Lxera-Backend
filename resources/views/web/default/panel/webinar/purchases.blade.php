    ...
    <section class="mt-25">
        @if (!empty($sales) and !$sales->isEmpty())
            <div
                ...
                <h2 class="section-title">{{ trans('panel.registered_courses') }} </h2>
            </div>
            @foreach ($sales as $sale)
                @php
                    ...
                    <section class="mb-80">
                        ...
                        @if ($sale->webinar->start_date > time())
                            @include(getTemplate() . '.includes.no-result', [
                                ...
                                'title' => trans('panel.courses_not_started_yet'),
                                'hint' => '',
                                'btn' => [
                       ...
                       @elseif (!$sale->webinar->isUserHasAccessToContent() && !$sale->webinar->groups->isEmpty())
                       <p class="text-center alert alert-warning" style="margin-top: 20px !important">
                           {{ trans('panel.content_unavailable') }}
                       </p>
                   @else
                            <div class="row mt-10">
                                <div class="col-12">
