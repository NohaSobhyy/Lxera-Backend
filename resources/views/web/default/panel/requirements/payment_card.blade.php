@php
    ...
    $mainTitle = trans('panel.early_registration_fee');
    $subTitle = trans('panel.price_includes_tax');
    ...
    @else
        ...
        @if (empty($bundleData['bundle']->class_id) && empty($bundleData['bundle']->bridging))
            <div class="text-center mx-auto mt-15">
                <p class="text-danger font-weight-bold">{{ trans('panel.or') }}</p>
                <a href="/panel/{{ $bundleData['bundle']->bundle->id }}/book_seat"
                    class="btn btn-outline-light register-btn mx-auto mt-5">{{trans('panel.reserve_seat_here')}} </a>
            </div>
        @endif
