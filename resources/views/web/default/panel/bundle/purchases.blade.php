    ...
    <section class="mt-25">
        @if (!empty($sales) and !$sales->isEmpty())
            <div
                ...
                <h2 class="section-title">{{ trans('panel.registered_programs') }} </h2>
            </div>
            @foreach ($sales as $sale)
                @php
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-30">
                            ...
                            <div style="display: flex; gap: 16px;">
                                @if ($item->content_table && $item->start_date <= time() && $hasActiveWebinars && $sale->access_to_purchased_item)
                                    <a href="{{ $item->content_table }}" class="btn btn-primary" target="_blank" style="font-size:18px">
                                        تحميل جدول المحاضرات <!-- You can customize the button text here -->
                                    </a>
                                @endif
                                @if ($item->academic_guide && $item->start_date <= time() && $hasActiveWebinars && $sale->access_to_purchased_item)
                                    <a href="{{ $item->academic_guide }}" class="btn btn-primary" target="_blank" style="font-size:18px">
                                        تحميل الدليل الاكاديمي <!-- You can customize the button text here -->
                                    </a>
                                @endif
                            </div>
                        </div>
            ...
            return optional($bundleWebinar->webinar)->start_date ?? PHP_INT_MAX;
                                            ...
                                            @endforeach
                                            <tr>
                                                <th colspan="7">{{ trans('panel.total_hours') }} : 
                                                    {{ $totalHours }}</th>
                                            </tr>
                                        </table>
                                    </div>
