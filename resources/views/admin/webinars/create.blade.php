                                        ...
                                        <div class="col-12 col-md-5">
                                            ...
                                            <div class="form-group mt-15 ">
                                                ...
                                                <select name="type"
                                                    ...
                                                    <option value="newGraduation_project"
                                                        @if (!empty($webinar) and $webinar->isNewGraduationProject() or old('type') == \App\Models\Webinar::$newGraduationProject) selected @endif>التقيم النهائي
                                                    </option>
                                                </select>
                                            ...
                                            <div class="form-group mt-15">
                                                <div class="d-flex">
                                                    ...
                                                    <input type="checkbox" name="hasGroup" id="hasGroup"
                                                        ...
                                                        @if (isset($webinar->hasGroup) && $webinar->hasGroup == 1) checked @endif value="1">
                                                ...
                                                <option {{ !empty($webinar) ? '' : 'selected' }} disabled>{{ trans('public.choose_category') }}</option>
                                                        @foreach ($categories as $category)
                                                            @if (!empty($category->subCategories) && count($category->subCategories))
                                                                {{-- Check if subcategories have their own subcategories (3-level hierarchy) --}}
                                                                @if ($category->subCategories->first()->subCategories->count() > 0)
                                                                    {{-- 3-level hierarchy (main > category > subcategory) --}}
                                                                    @foreach ($category->subCategories as $subCategory)
                                                                        @if ($subCategory->subCategories->count() > 0)
                                                <optgroup label="{{ $category->title }} > {{ $subCategory->title }}">
                                                                                @foreach ($subCategory->subCategories as $subSubCategory)
                                                <option value="{{ $subSubCategory->id }}" 
                                                                                        {{ (!empty($webinar) && $webinar->category_id == $subSubCategory->id) ? 'selected' : '' }}>
                                                                                        {{ $subSubCategory->title }}
                                                </option>
                                                                                @endforeach
                                                </optgroup>
                                                                        @else
                                                                            {{-- If category has subcategories but they don't have their own, make them selectable (2-level hierarchy) --}}
...
@push('scripts_bottom')
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const subCategoryOptions = document.querySelectorAll('.sub-category-option');
        
        // Add event listener to make only Subcategories clickable
        subCategoryOptions.forEach(option => {
            option.addEventListener('click', function(event) {
                // Allow the selection of the Subcategory
                event.preventDefault(); // Prevents the default selection behavior
                const selectedValue = event.target.value;
                
                // Optionally, you can highlight or show which option is clicked.
                console.log("Selected Subcategory ID: " + selectedValue);

                // Change the value of the select field (you can use this for submission)
                document.querySelector("#categories").value = selectedValue;
            });
        });
    });
</script>
