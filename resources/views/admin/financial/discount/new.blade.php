                    ...
                    <div class="row">
                        <div class="col-12 col-md-8 col-lg-6">
                            <form action="{{ getAdminPanelUrl() }}/financial/discounts/{{ !empty($discount) ? $discount->id.'/update' : 'store' }}" method="Post">
                                ...
                                <div class="form-group js-bundlesinstallment-input {{ (!empty($discount) and $discount->source == \App\Models\Discount::$discountSourceBundleinstallment) ? '' : 'd-none' }}">
                                    <label class="input-label">{{ trans('update.bundles') }}</label>
                                    <select name="bundleinstallment_ids[]" multiple="multiple" class="form-control search-bundle-select2 " data-placeholder="{{ trans('update.search_bundle') }}">

                                        @if(!empty($discount) and !empty($discount->discountInstallment))
                                            @foreach($discount->discountInstallment as $discountBundle)
                                                @if(!empty($discountBundle->bundle))
                                                    <option value="{{ $discountBundle->bundle->id }}" selected>{{ $discountBundle->bundle->title }}</option>
                                                @endif
                                            @endforeach
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="user_ids[]" multiple="multiple" class="form-control search-user_code-select2 select2"
                                        ...
                                        @if(!empty($userDiscounts) && $userDiscounts->count() > 0)
                                            @foreach($userDiscounts as $userDiscount)
                                                <option value="{{ $userDiscount->user_id }}" selected>{{ $userDiscount->user->full_name }} - {{$userDiscount->user->user_code}}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                </div>
