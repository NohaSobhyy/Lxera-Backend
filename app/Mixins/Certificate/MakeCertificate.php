...
class MakeCertificate
{
    ...
    public function makeBody($data)
    {
        ...
        // dd($student_name_length);


        $bodyData = [
            'student_name' => trim($data['student_name']) ?? '', // Default to empty string if not provided
    ...
    public function makeImage($certificateTemplate, $body)
    {
        ...
        if (isset($body['graduation_date'])) {
            ...
            $formattedDate2 =  $this->getOrdinal($day) . " " . $month . " " . $year;
        }
        ...
        if (isset($body['course_hours']) && $formattedDate) {
            ...
            if ($certificateTemplate->type == 'new_verssion_course') {
                $body['graduation_date'] =   $formattedDate2;
                $body['text'] =  $body['course_hours'] . " hours ";
            } else {
    ...
    public function saveCourseCertificate($user, $course, $template)
    {
        ...
        if (empty($certificate)) {
            $date = $group ? Carbon::parse($group->end_date) : $course->end_date;
            $year = $date->format('Y');
            $month = $date->format('m');
            $day = $date->format('d');
            $certificate = Certificate::create($data);
    ...
    public function makeBundleCertificate(Bundle $bundle, $format = 'png', $gpa, $allAssignmentsPassed)
    {
        ...
        if (!empty($template) && !empty($bundle)) {
            ...
            if ($allAssignmentsPassed && $gpa !== null) {
                ...
                $body['student_name'] = $user->student->en_name ?? '';
                $body['course_name'] = $bundle->bundle_name_certificate;
                $body['graduation_date'] = $userCertificate->graduation_date ?? $bundle->end_date; // Include the end date
                $body['gpa'] = $gpa;
