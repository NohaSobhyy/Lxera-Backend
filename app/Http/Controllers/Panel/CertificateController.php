...
class CertificateController extends Controller
{
    ...
    public function achievements(Request $request)
    {
        ...
        foreach ($userWebinars as $webinar) {
            ...
            $template = $webinar->certificate_template()
            ...
            $isStudentExceptionWebinar = StudentExceptionCertificate::where('student_id', $student->id) // use the student's ID
                ...
                if (!$group) {
                    ...
                    // Get the learning percentage for the student
                    $webinarStatisticController = new WebinarStatisticController();
                    $learningPercent = $webinarStatisticController->getCourseProgressForStudent($webinar, $user->id);
            
                    // If learning progress is greater than 50 and the student is eligible, generate the certificate
                   // dd($template);
                    if ($learningPercent == 100 && !empty($template) && !$isStudentExceptionWebinar) {
                       
                        $this->makeCourseCertificate($webinar->id);
                    }
                }
            
                // If the student is part of a group and all conditions are met, generate the certificate
        ...
        foreach ($userbundles as $bundle) {
           ...
           $bundlesHasGraduationProject = $bundle->bundleWebinars()
           ->whereHas('webinar', function ($query) {
               $query->whereHas('assignments')
                     ->whereIn('type', ['graduation_project', 'newGraduation_project']);
           })
           ->get();
//dd($bundlesHasGraduationProject);
