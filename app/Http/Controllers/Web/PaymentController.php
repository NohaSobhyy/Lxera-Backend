...
class PaymentController extends Controller
{
    ...
    public function payStatus(Request $request, $order_id = null)
    {
        ...
        if (!empty($order)) {
                ...
                try {
                    ...
                    if (!empty($order->orderItems->first()->webinar_id)) {
                        $user->update([
                         ...
                         $availableLicenses = (new MicrosoftGraphService)->getAvailableLicenses();
                //  dd($availableLicenses);
                   if (!empty(env('MICROSOFT_APPLY'))) {
                    if ($userFirstSale && $userFirstSale->id == $webinar_sale->id) {
                        $userEmailInTeams = $user->user_code . '@anasacademy.uk';
                        ...
                        if (!MicrosoftGraph::userExists($userEmailInTeams)) {
                            ...
                            if (isset($microsoftUser['id'])) {
                                $skuId = '314c4481-f395-4525-be8b-2ec4bb1e9d91';
                                $disabledPlans = [
                                    '6f23d6a9-adbf-481c-8538-b4c095654487', //
                                    'c4801e8a-cb58-4c35-aca6-f2dcc106f287', //
                                    'a9b86446-fa4e-498f-a92a-41b447e03337', // Skype for Business (Plan 2)
                                    '94065c59-bc8e-4e8b-89e5-5138d471eaff', // Sway
                                    'c68f8d98-5534-41c8-bf36-22fa496fa792', // StaffHub
                                    'a23b959c-7ce8-4e57-9140-b90eb88a9e97', // Stream
                                    'e03c7e47-402c-463c-ab25-949079bedb21', // Office for the Web
                                    '0a4983bb-d3e5-4a09-95d8-b2d0127b3df5', // SharePoint (Plan 1)
                                    'c87f142c-d1e9-4363-8630-aaea9c4d9ae5',//to do
                                    'a55dfd10-0864-46d9-a3cd-da5991a3e0e2',// project
                                    ...
                                    '0feaeb32-d00e-4d66-bd5a-43b5b83db82c', //MCOSTANDARD Skype for Business (Plan 2)
                                    '8c7d2df8-86f0-4902-b2ed-a0458298f3b3',// Deskless staffhup
                                ];
                                $licenseResponse = MicrosoftGraph::assignLicenseWithServicePlans($microsoftUser['id'], $skuId,$disabledPlans);
                                // 3. Verify license (wait 5 seconds for propagation)
                                sleep(3);
                                $userLicenses = MicrosoftGraph::getUserLicenses($microsoftUser['id']);
                                
                            } else {
                                dd('User creation failed', $microsoftUser);
                            }
                        } else {
            
                            $existingUser = MicrosoftGraph::getUserByEmail($userEmailInTeams);
                            if ($existingUser) {
                                $skuId = '314c4481-f395-4525-be8b-2ec4bb1e9d91';
                                $disabledPlans = [
                                    '6f23d6a9-adbf-481c-8538-b4c095654487', //
                                ...
                                $licenseResponse = MicrosoftGraph::assignLicenseWithServicePlans($existingUser['id'], $skuId,$disabledPlans);
                ...
                if (!empty(env('MICROSOFT_APPLY'))) {
                    if ($userFirstSale && $userFirstSale->id == $bundle_sale->id) {
                        ...
                        if (!MicrosoftGraph::userExists($userEmailInTeams)) {
                            ...
                            // User already exists - get their ID and assign license
                            $existingUser = MicrosoftGraph::getUserByEmail($userEmailInTeams);
                            if ($existingUser) {
                                $skuId = '314c4481-f395-4525-be8b-2ec4bb1e9d91';
                                $disabledPlans = [
            ...
            if (!empty($data['order']) && $data['order']->status === Order::$paid) {
              //  $this->sendWhatsAppMessage($user->mobile, "Thank you for your payment! Your order has been successfully processed.");
                $toastData = [
    ...
    {
      $url = 'https://live-mt-server.wati.io/7514/api/v1/sendTemplateMessages';  // Changed to template endpoint
      $apiKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..PK7tME3TFkheGmloVT5YaHgHQMcN3u4flefLGCqFGy4'; // API Key
   
      // Prepare the data for the WhatsApp message
      $data = [
          'receivers' => [
              ['whatsappNumber' => $phone], // User's phone number
              ['whatsappNumber' => $phone, 'customParams' => [
                  ['name' => 'name', 'value' => 'eman'], // You can replace with dynamic parameters
              ]],
          ],
          'template_name' => 'woman',  // Template name you want to use
          'broadcast_name' => 'wtest', // Broadcast name (can be anything relevant)
      ];
   
      // Log request details for debugging
      // \Log::info('Sending request to WATI API', [
      //     'url' => $url,
      //     'headers' => [
      //         'Authorization' => 'Bearer ' . $apiKey,
      //         'Content-Type' => 'application/json'
      //     ],
      //     'body' => json_encode($data), // Ensure body is JSON encoded
      // ]);
      ...
      try {
          ...
          $response = $client->post($url, [
              ...
              'json' => $data, // The JSON body to send in the request
          ]);
   
          $responseBody = json_decode($response->getBody(), true);
          // if (isset($responseBody['status']) && $responseBody['status'] == 'success') {
          //     // Log or process the successful message send here
          //     \Log::info('WhatsApp message sent successfully');
          // } else {
          //     // Handle failure response from WATI API
          //     \Log::error('Failed to send WhatsApp message: ' . json_encode($responseBody));
          // }
      } catch (\Exception $e) {
