...
class AssignmentHistoryController extends Controller
{
    ...
    public function setGrade(Request $request, $assignmentId, $historyId)
    {
        ...
        if (!empty($assignment)) {
            ...
            // Ensure the user is the owner of the webinar
            if (!empty($webinar) && ($webinar->isOwner($user->id) or $webinar->isPartnerTeacher($user->id))) {
                $studentId = $request->get('student_id');

                // Get the assignment history
                $assignmentHistory = $this->getAssignmentHistory($webinar, $assignment, $user, $studentId);

                // Check if the assignment history exists and is valid
                if (
                    !empty($assignmentHistory) && $historyId == $assignmentHistory->id &&
                    ($assignmentHistory->instructor_id == $user->id || $webinar->isPartnerTeacher($user->id))
                ) {
                    ...
                    $rules = [
                        ...
                        'comment' => 'nullable|string',  // Adding validation for the comment field
                    ];
                    ...
                    $grade = $data['grade'];
                    $comment = $data['comment'] ?? null;  // Capture the comment, default to null if not provided

                    $status = WebinarAssignmentHistory::$passed;

                    // Check if the grade is below the passing grade
                    if ($grade < $assignment->pass_grade) {
                        $status = WebinarAssignmentHistory::$notPassed;
                    }

                    // Update the assignment history with grade, status, and comment
                    $assignmentHistory->update([
                        'status' => $status,
                        'grade' => $grade,
                        'comment' => $comment,  // Save the comment to the database
                    ]);

                    // If the assignment is passed, reward the student
        ...
        try {
            ...
            foreach ($bundleWebinars as $bundleWebinar) {
                ...
                if ($isNewGraduationProject) {
                    ...
                    $totalStudentGrade = 0;
                    foreach ($assignments as $assignment) {
                        $history = $assignment->assignmentHistory;
                        if ($history && isset($history->grade)) {
                            $totalStudentGrade += $history->grade;
                        }
                    }
                    $finalPercentage = $totalStudentGrade;
                    $gpa = $this->convertGradeToGPA($finalPercentage);
                } else {
                    // Old graduation project calculation (average of grades)
                    foreach ($assignments as $assignment) {
                        if ($assignment->assignmentHistory) {
                            $totalGrade += $assignment->assignmentHistory->grade;
                            $totalCount++;
                ...
                $results[] = [
                    ...
                    'graduationProjectWebinars' => $graduationProjectWebinars,
                    'assignments' => $assignments,
                    'totalGrade' => $isNewGraduationProject ? $totalStudentGrade : $totalGrade,
                    'totalCount' => $totalCount,
                    'calculation_type' => $isNewGraduationProject ? 'newGraduation_project' : 'graduation_project',
                    'gpa' => $gpa
                ];
            }
    ...
    public function downloadAttach($assignmentId, $historyId, $messageId)
    {
        ...
        if (!empty($assignment)) {
            ...
            if (!empty($webinar) and $webinar->checkUserHasBought($user)) {
                $allow = ($webinar->creator_id == $user->id or $webinar->teacher_id == $user->id or $user->isAdmin() or $webinar->isPartnerTeacher($user->id));
