...
class WebinarController extends Controller
{
    ...
    {
        // Get unattached webinars and format them
        // $webinars = Webinar::where('unattached', 1)
        //     ->get()
        //     ->map(function ($webinar) {
        //         return $this->formatWebinar($webinar);
        //     });
        ...
        // Get categories with their unattached webinars
        $categories = Category::whereHas('webinars', function($query) {
            ...
            ->map(function ($category) {
                return [
                    ...
                    'slug' => $category->slug,
                    'webinars' => $category->webinars->map(function ($webinar) {
                        return $this->formatWebinar($webinar);
                    })
                ];
            });
    ...
    //   public function index()
    // {
    //     // Retrieve categories with webinars attached, including subcategories and sub-subcategories
    //     $categories = Category::whereNull('parent_id') // Get only parent categories
    //         ->with([
    //             'webinars' => function ($query) {
    //                 $query->where('unattached', 1); // Filter webinars that are unattached
    //             },
    //             'subCategories.subCategories' => function ($query) {
    //                 $query->orderBy('order', 'asc'); // Optionally order subcategories
    //             },
    //             'subCategories.webinars' => function ($query) {
    //                 $query->where('unattached', 1); // Filter webinars for subcategories
    //             },
    //             'subCategories.subCategories.webinars' => function ($query) {
    //                 $query->where('unattached', 1); // Filter webinars for sub-subcategories
    //             }
    //         ])
    //         ->orderBy('order', 'asc') // Order main categories
    //         ->get();
        
    //     // Format categories for the frontend with webinars included
    //     $formattedCategories = $categories->map(function ($category) {
    //         return $this->formatCategoryWithWebinars($category);
    //     });
        
    //     return response()->json(['categories' => $formattedCategories]);
    // }
    
    // private function formatCategoryWithWebinars($category)
    // {
    //     // Format the subcategories and their webinars
    //     $subCategories = $category->subCategories->map(function ($subCategory) {
    //         return $this->formatSubCategoryWithWebinars($subCategory);
    //     });
    
    //     return [
    //         'id' => $category->id,
    //         'title' => $category->title,
    //         'webinars' => $category->webinars, // Include webinars for the main category
    //         'subCategories' => $subCategories
    //     ];
    // }
    
    // private function formatSubCategoryWithWebinars($subCategory)
    // {
    //     // Format the sub-subcategories and their webinars
    //     $subSubCategories = $subCategory->subCategories->map(function ($subSubCategory) {
    //         return [
    //             'id' => $subSubCategory->id,
    //             'title' => $subSubCategory->title,
    //             'webinars' => $subSubCategory->webinars // Include webinars for sub-subcategories
    //         ];
    //     });
    ...
    //         'webinars' => $subCategory->webinars, // Include webinars for this subcategory
...
private function formatCategoryWithWebinars($category)
{
    ...
    // Format the webinars for the main category
    $formattedWebinars = $category->webinars->map(function ($webinar) {
        return $this->formatWebinar($webinar);
    });
    ...
    return [
        'id' => $category->id,
        'title' => $category->title,
        'webinars' => $formattedWebinars, // Include formatted webinars for the main category
        'subCategories' => $subCategories
    ];
}
...
private function formatSubCategoryWithWebinars($subCategory)
{
    ...
    $subSubCategories = $subCategory->subCategories->map(function ($subSubCategory) {
        return [
            'id' => $subSubCategory->id,
            'title' => $subSubCategory->title,
            'webinars' => $subSubCategory->webinars->map(function ($webinar) {
                return $this->formatWebinar($webinar); // Format webinars for sub-subcategory
            }) // Include formatted webinars for sub-subcategories
        ];
    });

    // Format the webinars for the current subcategory
    $formattedWebinars = $subCategory->webinars->map(function ($webinar) {
        return $this->formatWebinar($webinar); // Format webinars for subcategory
    });

    return [
        'id' => $subCategory->id,
        'title' => $subCategory->title,
        'webinars' => $formattedWebinars, // Include formatted webinars for this subcategory
        'subCategories' => $subSubCategories
    ];
}

// Format each webinar's details using the `formatWebinar` method
private function formatWebinar($webinar)
{
    return [
        'id' => $webinar->id,
        'title' => $webinar->title,
        ...
        'start_date' => dateTimeFormat($webinar->start_date, 'j M Y'), 
        'created_at' =>dateTimeFormat($webinar->created_at, 'j M Y'), 
        'applying_link' => url("/webinars/{$webinar->id}/apply") // Generate the applying link dynamically
    ];
}
private function formatbundle($bundle)
{
    return [
        'id' => $bundle->id,
        'title' => $bundle->title,
        ...
        'start_date' => dateTimeFormat($bundle->start_date, 'j M Y'), 
        'created_at' =>dateTimeFormat($bundle->created_at, 'j M Y'), 
         // Generate the applying link dynamically
    ];
}
