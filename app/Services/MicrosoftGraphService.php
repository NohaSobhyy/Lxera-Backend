...
class MicrosoftGraphService
{
    ...
    {
        $token = $this->getAccessToken();
        $response = $this->client->get("https://graph.microsoft.com/v1.0/users/$email", [
            'headers' => [
                'Authorization' => 'Bearer ' . $token
            ]
        ]);
    ...
    public function assignLicenseWithServicePlans($userId, $skuId, $disabledServicePlans = [])
    {
        $token = $this->getAccessToken();
        ...
        $response = $this->client->post("https://graph.microsoft.com/v1.0/users/$userId/assignLicense", [
            ...
            'json' => [
                'addLicenses' => [
                    [
                        'skuId' => $skuId,
                        'disabledPlans' => $disabledServicePlans, // ✅ This is supported
                    ]
                ],
                'removeLicenses' => []
            ],
